...
class Trenex:
    ...
    # -------------------------------------------------------------------------
    # Plugin Factory Features Exposed via Trenex (static methods)
    # -------------------------------------------------------------------------

    @staticmethod
    def create_plugin_template(plugin_name: str, plugin_type: str, output_dir: str = ".") -> str:
        """
        Create a plugin template. This method calls the plugin_factory under the hood.
        Returns the path to the created plugin template folder.
        """
        template_path = create_template(plugin_name, plugin_type, output_dir)
        logger.info(f"Plugin template for '{plugin_name}' created at {template_path}")
        return template_path

    @staticmethod
    def package_plugin(plugin_folder: str, output_dir: str = ".") -> str:
        """
        Package an existing plugin template folder into a .plg file.
        Returns the path to the packaged .plg file.
        """
        plg_path = package_plugin(plugin_folder, output_dir)
        logger.info(f"Plugin template at '{plugin_folder}' packaged as {plg_path}")
        return plg_path

    # -------------------------------------------------------------------------
    # (Remaining builder methods for loading plugins, connecting, building, etc.)
    # -------------------------------------------------------------------------